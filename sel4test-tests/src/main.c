...
main(int argc, char **argv)
{
    ...
    /* FILE *f = fopen("/home/axjllt/Desktop/file.txt", "w");
    if (f == NULL)
    {
    ...
    // seL4_MessageInfo_t info_reply, info_wait, info_send_wait, info_tmnt;
    seL4_MessageInfo_t info_reply, info_tmnt, info_consumer, info_producer;
    int reply, id;

    seL4_MessageInfo_t info_init = seL4_MessageInfo_new(INIT, 0, 0, 1);
    printf("seL4_Call from sel4test-test\n");
    info_reply = seL4_Call(endpoint, info_init);
    id = seL4_GetMR(0);
    reply = seL4_GetMR(1);
    ...
    if (id % 2 == 1) {
        ...
        for (int i = 0;i < 4;i ++) {
            ...
            seL4_SetMR(0, id);
            seL4_SetMR(1, i);
            info_reply = seL4_Call(endpoint, info_producer);
        }
    } else {
        /* This is a consumer */
        for (int i = 0;i < 4;i ++) {
            ...
            seL4_SetMR(0, id);
            seL4_SetMR(1, i);
            info_reply = seL4_Call(endpoint, info_consumer);
        }
    }
...
/*
    if (reply) {
        info_wait = seL4_MessageInfo_new(WAIT, 0, 0, 1);
        seL4_SetMR(0, id);
        info_reply = seL4_Call(endpoint, info_wait);
        // printf("!! %d Get reply from wait.\n", id);
    }

    for (int i = 0;i < 1050;i ++) {
        seL4_SetMR(0, id);
        seL4_SetMR(1, i);
        info_send_wait = seL4_MessageInfo_new(SEND_WAIT, 0, 0, 2);
start = rdtsc();
        info_reply = seL4_Call(endpoint, info_send_wait);
end = rdtsc();
data[i] = (end - start);
    }
*/

    /* For later display benchmark measurement */
    // for (int i = 0;i < 1050;i ++)
        // printf("\n%d %llu\n", id, data[i]);
    ...
    seL4_SetMR(0, id);
    info_reply = seL4_Call(endpoint, info_tmnt);
    printf("The end of client %d\n", id);
